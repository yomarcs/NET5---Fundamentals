...
WORKDIR /src
COPY ["Net5.Fundamentals.AWSLambda.csproj", "Net5.Fundamentals.AWSLambda/"]
RUN dotnet restore "Net5.Fundamentals.AWSLambda/Net5.Fundamentals.AWSLambda.csproj"

WORKDIR "/src/Net5.Fundamentals.AWSLambda"
COPY . .
RUN dotnet build "Net5.Fundamentals.AWSLambda.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Net5.Fundamentals.AWSLambda.csproj" \
...
Execute unit tests
```
cd "Net5.Fundamentals.AWSLambda/test/Net5.Fundamentals.AWSLambda.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "Net5.Fundamentals.AWSLambda/src/Net5.Fundamentals.AWSLambda"
